	...
	<div id="coummuBoardMain">
					...
					<tbody>
						...
						<tr>
							...
							<td style="float: left ;line-height: 14px;">
								<a href="commuView?num=${vo.num}&up_cate=3">${vo.b_subject} 
		...
		<ul id="MBPaging">
               <c:if test="${pageVO.pageNum>1}">
					<li>
						<a href='commuFreeBoard?pageNum=${pageVO.pageNum-1}&up_cate=3<c:if test="${pageVO.searchWord!=null && pageVO.searchWord!='' }">&searchKey=${pageVO.searchKey}&searchWord=${pageVO.searchWord }</c:if>'>이전</a></li>
				</c:if>
				<c:if test="${pageVO.pageNum<=1}">
					<li>이전</li>
				</c:if>
				<c:forEach var="p" begin="${pageVO.startPageNum }" end="${pageVO.startPageNum+pageVO.onePageNum-1 }">
					<c:if test="${p<=pageVO.totalPage }">
						...
						<!-- 현재페이지일떄 -->
							<li style="background-color:#d9d9d9"><a href="commuFreeBoard?pageNum=${p}&up_cate=3<c:if test="${pageVO.searchWord!=null && pageVO.searchWord!='' }">&searchKey=${pageVO.searchKey}&searchWord=${pageVO.searchWord }</c:if>">${p }</a></li>
						</c:if>
						<c:if test="${p!=pageVO.pageNum }">
						<!-- 현재페이지가 아닐때 -->
							<li><a href="commuFreeBoard?pageNum=${p}&up_cate=3<c:if test="${pageVO.searchWord!=null && pageVO.searchWord!='' }">&searchKey=${pageVO.searchKey}&searchWord=${pageVO.searchWord }</c:if>">${p }</a></li>
						</c:if>
					</c:if>		
				</c:forEach>			
				<c:if test="${pageVO.pageNum<pageVO.totalPage}">
					<li>
						<a href="commuFreeBoard?pageNum=${pageVO.pageNum+1}&up_cate=3<c:if test="${pageVO.searchWord!=null && pageVO.searchWord!='' }">&searchKey=${pageVO.searchKey}&searchWord=${pageVO.searchWord }</c:if>">다음</a></li>
				</c:if>	
		...
		<div>
			...
			<div id="commuSearch">
				<form method="get" action="commuFreeBoard" id="commuForm">
