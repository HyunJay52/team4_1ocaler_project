		...
		function setMyAllReview(num){
			...
			$.ajax({
				...
				success : function(result){
					console.log(result);
					var kategorie = $("#reviewKategorie option:selected").val();
					var tag = "<tr><td>구분</td><td>평가</td><td>제목<br/>아이디</td><td>내용</td><td>날짜</td><td>수정</td></tr>";
					var index = 0;
					result.list.forEach(function(data, idx){
						if(data.kategorie == 'mem_share'){
							tag += "<tr><td>회원간 거래</td>";
						}else if(data.kategorie == 'grouplocal'){
							tag += "<tr><td>모집</td>";
						}else{
							tag += "<tr><td>파머 직거래</td>";							
						}
						...
						tag += "<td><a href=''>"+data.re_subject+"</a><br/>"+data.sellerid+"</td>";
						tag += "<td>"+data.re_content+"</td>";
		...
		function myinfoReviewUpdate(data, num, rateCheck){
			$.ajax({
				...
				success : function(result){
					...
					$('.modal-backdrop').remove();
					$("#reviewMd").hide();
					$("#reviewMd").attr('class', 'modal fade');
					$("#reviewMd").attr('aria-modal', null);
					$("#reviewMd").attr('aria-hidden', true);
		...
		<div class="reviewBottom">
			...
			<div class="dealDateForm" style="width:400px;">
				...
				<select name="kategorie" id="reviewKategorie">
					<option value="mem_share" selected>회원간 거래</option>
					<option value="grouplocal">모집</option>
					<option value="sell_item">파머 직거래</option>
				</select>
			</div>
