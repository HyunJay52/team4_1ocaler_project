...
<script>	
		$(function(){
			...
			function searchPlaces(){
				var keyword = $("#eatViewInnerSearchWord").val();
				console.log(keyword);
			...
			var nullCheck = $("#eatViewGroupList div:nth-child(4)").text();
			...
			<c:forEach var="markers" items="${eatList}">
				geocoder.addressSearch('${markers.g_loc1}', function(result, status) {
				    // 정상적으로 검색이 완료됐으면 
				     if (status === kakao.maps.services.Status.OK) {
				        ...
				        var content = '<div class="customOverlay"><a href="eatViewPage?num=${markers.num }&loc_gu=${pageVO.loc_gu}&pageNum=${pageVO.pageNum}&category=한끼미식회<c:if test="${pageVO.searchWord!=null && pageVO.searchWord!='' }">&searchKey=${pageVO.searchKey}&searchWord=${pageVO.searchWord }</c:if>">${markers.g_subject}</a></div>'
...
<body id="eatViewBody">
	...
	<div class="groupSearchFrm">
			...
			<input type="image" id="eatViewSearchFrm" src="<%=request.getContextPath()%>/img/groupImg/search.png" value="검색"/>
	</div>
	...
	<div id="eatViewShowFrm">	
		...
		<ul id="eatViewGroupList" class="groupListView">
			<c:forEach var="vo" items="${eatList }">
				<a href="eatViewPage?num=${vo.num }&loc_gu=${pageVO.loc_gu}&pageNum=${pageVO.pageNum}&category=한끼미식회<c:if test="${pageVO.searchWord!=null && pageVO.searchWord!='' }">&searchKey=${pageVO.searchKey}&searchWord=${pageVO.searchWord }</c:if>"><li><!-- 나중에 searchkey, searchWord도 달고다녀야함 -->
					<div>
						...
						<img src="<%=request.getContextPath()%>/img/groupImg/dishB.png"/><span>${vo.down_cate }</span>
						<c:if test="${logId!=null }">
